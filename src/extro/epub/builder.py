...
def safe_epub_filename(title: str, *, fallback: str) -> str:
    ...
    cleaned = _SAFE_FILENAME_RE.sub("_", title).strip(" ._")
...
def build_epub(
    ...
    *,
    ...
    archive_opf_path = f"OEBPS/{opf_href}"
    ...
    try:
        with zipfile.ZipFile(temp_path, mode="w") as archive:
            ...
            archive.write(opf_path, archive_opf_path)
            for item in items:
                source_path = _safe_source_path(files_dir, item.href)
                ...
                if _is_html_item(item):
                    content = _clean_html(
                        ...
                        href=item.href,
                        title=title,
                        css_hrefs=[css_item.href for css_item in css_items],
                    )
                    _validate_html(content, item.href)
                    archive.writestr(archive_path, content)
                else:
                    archive.write(source_path, archive_path)
            _validate_archive_manifest(archive, items)
    ...
    return EpubBuildResult(
        ...
        html_file_count=len(html_items),
